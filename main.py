...
# from ops.dataset_3d import TSNDataSet
...
def main():
    ...
    if args.tune_from:
        ...
        sd = torch.load(args.tune_from, weights_only=False)
        sd = sd['state_dict']
...
def train(train_loader, model, criterion, optimizer, epoch, log, tf_writer):
    ...
    for i, (input, target) in enumerate(tqdm(train_loader, ncols=100)):
        ...
        if i % args.print_freq == 0:
            output = ('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                      ...
                      'Semantic Loss {semantic_loss:.4f}\t'
                      'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                      'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                epoch, i, len(train_loader), batch_time=batch_time,
                data_time=data_time, loss=losses, semantic_loss=semantic_loss if args.semantic else 0,
