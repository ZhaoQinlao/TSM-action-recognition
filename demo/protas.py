...
class MultiStageModel(nn.Module):
    def __init__(self, num_stages, num_layers, num_f_maps, dim, num_classes, causal=False, use_graph=True, **graph_args):
        ...
        self.stages = nn.ModuleList([copy.deepcopy(SingleStageModel(num_layers, num_f_maps, num_classes, num_classes, causal)) for s in range(num_stages-1)]) # 此处deepcopy有些冗余了，并不会出现参数共享问题
...
# Define the DonePrdictorModel class
class DonePredictorModel(nn.Module):
    def __init__(self, num_stages, num_layers, num_f_maps, dim, num_classes, causal=False, use_graph=True, **graph_args):
        super().__init__()
        self.stage1 = DoneProcessModel(num_layers, num_f_maps, dim, num_classes, causal)
        self.stages = nn.ModuleList([copy.deepcopy(DoneProcessModel(num_layers, num_f_maps, num_classes, num_classes, causal)) for s in range(num_stages-1)]) # 此处deepcopy有些冗余了，并不会出现参数共享问题
        self.use_graph = use_graph
        if use_graph:
            self.graph_learner = TaskGraphLearner(**graph_args)

    def forward(self, x, mask):
        out, out_app, out_done = self.stage1(x, mask)
        outputs = out.unsqueeze(0)
        outputs_app = out_app.unsqueeze(0)
        outputs_done = out_done.unsqueeze(0)
        for s in self.stages:
            out, out_app, out_done = s(F.softmax(out, dim=1) * mask[:, 0:1, :], mask)
            outputs = torch.cat((outputs, out.unsqueeze(0)), dim=0)
            outputs_app = torch.cat((outputs_app, out_app.unsqueeze(0)), dim=0)
            outputs_done = torch.cat((outputs_done, out_done.unsqueeze(0)), dim=0)
        return outputs, outputs_app
...
# Define the DoneProcessModel class
class DoneProcessModel(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes, causal=False):
        super().__init__()
        self.conv_1x1 = nn.Conv1d(dim, num_f_maps, 1)
        self.layers = nn.ModuleList([copy.deepcopy(DilatedResidualLayer(2 ** i, num_f_maps, num_f_maps, causal=causal)) for i in range(num_layers)])
        ...
        self.prob_fusion = ProbabilityProgressFusionModel(num_classes)
        ### Done Process module
        self.conv_done = nn.Conv1d(num_f_maps, num_classes, 1)
        self.conv_done_out = nn.Conv1d(num_classes, num_classes, 1)
    ...
    def forward(self, x, mask):
        ...
        out = self.prob_fusion(prob_out, progress_out)
        out = out * mask[:, 0:1, :]
        done_out = F.relu(self.conv_done(out)) * mask[:, 0:1, :]
        done_out = self.conv_done_out(done_out) * mask[:, 0:1, :]
        return out, progress_out, done_out
...
## main api
mapping_file = '/home/fitz_joye/TSM-action-recognition/data/category.txt'
graph_path = '/home/fitz_joye/ProTAS/data/assembly101/graph/graph.pkl'
